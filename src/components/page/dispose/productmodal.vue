<template>
    <div class="productmodal">
        <el-dialog :title="tit" :destroy-on-close="isclose" :visible.sync="dialogFormVisible" width="60%" :before-close="beforclose" center>
                ...
                <el-form id="addProduct" :model="form" ref="form" :rules="rules">
                    ...
                    <el-col :span="24" v-if="this.tableData1.length > 0">
                        <el-table :data="tableData1" border style="width: 100%">
                            <el-table-column v-for="item in columnList" :key="item.id" :prop="item.prop" :label="item.label" align="center">
                            </el-table-column>

                            <el-table-column label="操作" align="center" width="150">
                                <template slot-scope="scope">
                                    <el-button type="info" plain @click="handleEdit1(scope.$index, scope.row)">修改</el-button>
                                    <el-button type="danger" plain class="red" @click="handledistribute(scope.row.id)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                    </el-col>
            ...
            <el-dialog width="50%" title="物料修改" :visible.sync="innerVisible" append-to-body>
                <el-row>
                        ...
                        if (res.code === '0') {
                            const cities = res.data.itemList.map((value, i) =>
                                value.dataIsEmpty == 'true'
                                    ...
                                    : { value: value.id, label: value.itemName, leaf: node.level >= 0 }
                            );
                            resolve(cities);
                        }
                    });
