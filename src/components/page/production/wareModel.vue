 ...
 * @LastEditTime: 2021-06-29 10:33:58
 * @FilePath: \vue-manage-system\src\components\page\production\wareModel.vue
-->
<template>
  <div class="wareModel">
                ...
                >
                 ...
                 <el-table-column label="操作" width="200" >
                            <template slot-scope="scope">
                                <!-- <el-button type="add" v-if="scope.row.state==1 || scope.row.state==3" @click="hlandpur(scope.row,'2')" plain>采购</el-button> -->
                               <el-button type="success" v-if="scope.row.state=='1' || scope.row.state==2" @click="hlandpur(scope.row,'3')" plain>已就绪</el-button>
                               <el-button type="info" v-if="scope.row.state==3 " @click="hlandpur(scope.row,'1')" plain>取消操作</el-button>
                            </template>
                    </el-table-column>
         </el-table>
         <div class="page">
...
import { produceWarehousePartPage,produceWarehousePartPut } from 'api/index'
export default {
    name: 'wareModel',
    props:{
        dialogFormVisible:{
    ...
    methods: {
        handleCurrentChange(val){
            ...
            this.getproduceWarehousePartPage()
        },
        hlandpur(info,num){
            let obj = {
                id:info.id,
                ...
                partSource:info.partSource,
                partId:info.partId,
                produceWarehouseId:info.produceWarehouseId
            }
            produceWarehousePartPut(obj).then(res=>{
                if(res.code=='0'){
       ...
       async getproduceWarehousePartPage(){
          await produceWarehousePartPage({produceWarehouseId:this.purid.id,...this.page,state:this.stateval}).then(res=>{
               if(res.code=='0'){
      ...
      async getpro(info){
           this.purid = info
          await this.getproduceWarehousePartPage()
       },
       close(num){
           this.init()
           this.$emit('close',num)
