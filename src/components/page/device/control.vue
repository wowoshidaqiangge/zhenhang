      ...
      <el-tab-pane :label="item.deviceType" v-for="(item, index) in runlist" :key="index" class="conlist">
        <div class="con">
          <div class="row-bg" v-for="h in runlistinfo" :key="h.id">
            <div class="p1">
              <span class="icon"></span><span class="tit"> {{ h.deviceName }}</span><span class="img"><img
                  src="~@/assets/img/wifi.png" /></span>
            </div>
            <div class="p1">
              <p>
            ...
            <div class="p1">
              ...
              <p>
                <span v-if="h.yieldCollect===1"> <span>当日产量:</span><span v-if='h.dva|| h.dva ===0'>
                    {{  h.dva + '件' }}</span></span>
              </p>
            </div>
            <div class="p1">
              ...
              <p>
                <span v-if="h.yieldCollect===1"><span>累计产量:</span><span>
                    {{ h.totalProduce ? h.totalProduce + '件' : '--' }}</span></span>
              </p>
            </div>
          </div>
        </div>
